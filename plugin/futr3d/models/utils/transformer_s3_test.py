        ...
        **kwargs,
        ...
        Args:
            ...
            reference_points (Tensor): The reference
                ...
                (bs, num_query, 4) when as_two_stage,f
        ...
        for lid, layer in enumerate(self.layers):
            ...
            (
                ...
                img_output,
                pts_output, 
                cam_ncscore, lidar_ncscore, p_cam, p_lidar, img_centers_idx, pts_centers_idx, top3_img_values, top3_img_indices, top3_pts_values, top3_pts_indices, cam_consistency, lidar_consistency, cam_factor,lidar_factor  
            ) = layer(  # Call transformer.py
                output,
                *args,
                layer_id=lid,
                query_pos=query_pos,
                ...
                prev_cam_consistency = prev_cam_consistency,
                prev_lidar_consistency = prev_lidar_consistency,
                prev_cam_factor = prev_cam_factor,
                prev_lidar_factor = prev_lidar_factor,
                **kwargs,
            )
            
            prev_cam_consistency = cam_consistency
            prev_lidar_consistency = lidar_consistency
            prev_cam_factor = cam_factor
            prev_lidar_factor = lidar_factor
            
            img_pts_output_dict[f"img{lid}"] = img_output
            img_pts_output_dict[f"pts{lid}"] = pts_output
            
            ncscore_dict[f"img{lid}"] = cam_ncscore
            ncscore_dict[f"pts{lid}"] = lidar_ncscore
            
            p_val_dict[f"img{lid}"] = p_cam
            p_val_dict[f"pts{lid}"] = p_lidar

            center_dict[f"img{lid}"] = img_centers_idx
            center_dict[f"pts{lid}"] = pts_centers_idx
            
            top3_values_dict[f"img{lid}"] = top3_img_values
            top3_values_dict[f"pts{lid}"] = top3_pts_values
            
            top3_indices_dict[f"img{lid}"] = top3_img_indices
            top3_indices_dict[f"pts{lid}"] = top3_pts_indices
            
            # Compute top1 stability 
            if lid == 5 and not is_stage2:
                img_layers = torch.stack([center_dict["img0"], center_dict["img1"], center_dict["img2"], center_dict["img3"], center_dict["img4"], center_dict["img5"]])
                pts_layers = torch.stack([center_dict["pts0"], center_dict["pts1"], center_dict["pts2"], center_dict["pts3"], center_dict["pts4"], center_dict["pts5"]])
                
                img_changes = torch.sum(img_layers[1:] != img_layers[:-1], dim=0)
                pts_changes = torch.sum(pts_layers[1:] != pts_layers[:-1], dim=0)
                
                img_cons_factor = 100 * (1 - img_changes.float() / 6.)
                pts_cons_factor = 100 * (1 - pts_changes.float() / 6.)
                
                p_val_dict["img5"] = p_val_dict["img5"] / (p_val_dict["img5"] + p_val_dict["pts5"])
                p_val_dict["pts5"] = p_val_dict["pts5"] / (p_val_dict["img5"] + p_val_dict["pts5"])
 
                p_val_dict["img5"] = p_val_dict["img5"] * img_cons_factor
                p_val_dict["pts5"] = p_val_dict["pts5"] * pts_cons_factor

                # Use p-val for alpha and beta 
                alpha = p_val_dict["img5"] / (p_val_dict["img5"] + p_val_dict["pts5"])
                beta = p_val_dict["pts5"] / (p_val_dict["img5"] + p_val_dict["pts5"])
                
                alpha = torch.where(alpha >= 0.7, torch.tensor(0.5), alpha) 
                beta =  torch.where(beta < 0.3, torch.tensor(0.5), beta) 
        ...
        ):  # lid = 5, this is true so both features are not retured
            return (
        ...
        return (
            ...
            top3_indices_dict,
            alpha, beta
        )  # Return to FUTR3DHead line 205 @futr3d_head.py
...
class FUTR3DTransformer(BaseModule):
    ...
    def init_layers(self):
        ...
        if self.as_two_stage:
            ...
            self.pofs_trans_norm = nn.LayerNorm(self.embed_dims * 2)
    ...
    def forward(
        ...
        **kwargs,
        ...
        (
            ...
            top3_indices_dict,
            alpha, beta
        ) = self.decoder(  # call FUTR3DTransformerDecoder @ futr3d_transformer.py
            query=query,
            key=None,
