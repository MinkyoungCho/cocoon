...
class FUTR3DHead(DETRHead):
    ...
    def forward(
        self, mlvl_pts_feats, mlvl_img_feats, radar_feats, img_metas, is_test=False
    ):
        """Forward function.
        Args:
        ...
        return outputs_classes, outputs_coords, aux_outs, img_pts_output_dict,
    ...
    def loss(
        ...
        losses_cls, losses_bbox = multi_apply( # losses_cls.shape = [6], losses_bbox.shape = [6] --> loss value per each decoder layer 
            self.loss_single,
            all_cls_scores, # torch.Size([6, 1, 900, 10])
            all_bbox_preds, # torch.Size([6, 1, 900, 10])
            all_gt_bboxes_list, # len: 6
            all_gt_labels_list,  # len: 6
            img_metas_list, # len: 6
            all_gt_bboxes_ignore_list, # len: 6
    ...
    def auxiliary_loss_single(
        ...
        if self.run_mode == "calib":
            self.save_calib_ncscores(img_feats_aligned, pts_feats_aligned, labels)

        # 1. cross-modal center loss
        ...
        #     wandb.log({"loss_center": loss_center, "loss_geomed": loss_geomed, "loss_sep": loss_sep})
    ...
    def loss_single(
        ...
        labels = torch.cat(labels_list, 0)  # label_list[0].shape = torch.Size([900])
        label_weights = torch.cat(label_weights_list, 0)
        bbox_targets = torch.cat(bbox_targets_list, 0)
        bbox_weights = torch.cat(bbox_weights_list, 0)
    ...
    def save_calib_ncscores(self, img_feats_aligned, pts_feats_aligned, labels):
        """
        TODO: Implement NC score computation here
           If test code and last layer, compute NC score
           How to get/load feature aligner network and center coordinates which are pretrained?
        """
        centers = self.center_loss.centers  # [10, 128]
        print(f"[NC Score: centers] {centers}")
        print(
            f"[NC Score: feature_aligner.fc1.weight] {self.feature_aligner.fc1.weight}"
        )
        print(">>> Same?")  # This is same with the model config file (load_from)
        ...
        num_queries = img_feats_aligned.size(0)
        labels = labels.unsqueeze(1).expand(num_queries, self.num_classes)
        mask = labels.eq(classes.unsqueeze(0))

        centers = centers.unsqueeze(0).expand(num_queries, -1, -1)
        centers_masked = centers[mask]

        img_feats_aligned_masked = img_feats_aligned[mask.sum(dim=1) == 1]
        pts_feats_aligned_masked = pts_feats_aligned[mask.sum(dim=1) == 1]

        idx = mask.sum(dim=1).nonzero()[0][0]
        cls_idx = labels[idx][0]
        assert (
            img_feats_aligned_masked[0]
            == img_feats_aligned[mask.sum(dim=1).nonzero()[0][0]]
        ).all()
        assert (
            pts_feats_aligned_masked[0]
            == pts_feats_aligned[mask.sum(dim=1).nonzero()[0][0]]
        ).all()
        assert (centers_masked[0] == centers[1][cls_idx]).all()

        assert mask.sum() == img_feats_aligned_masked.shape[0]
        assert mask.sum() == pts_feats_aligned_masked.shape[0]
        assert mask.sum() == centers_masked.shape[0]

        ncscore_img = torch.norm(
            img_feats_aligned_masked - centers_masked, p=2, dim=1
        )  # [18]
        ncscore_pts = torch.norm(
            pts_feats_aligned_masked - centers_masked, p=2, dim=1
        )  # [18]

        with open("ncscores_calib_cam_layer1.txt", "a") as file:
            for i in range(len(ncscore_img)):  # 18 items!
                file.write(f"{ncscore_img[i]:.4f} ")
            file.write(f"\n")

        with open("ncscores_calib_lidar_layer1.txt", "a") as file:
            for i in range(len(ncscore_pts)):
                file.write(f"{ncscore_pts[i]:.4f} ")
            file.write(f"\n")