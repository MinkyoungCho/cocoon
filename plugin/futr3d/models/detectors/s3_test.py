...
class FUTR3D(MVXTwoStageDetector):
    def __init__(
        ...
        if self.run_mode == "calib":
            print("[CHECK] loss.backward() in optimizer.py")
                ...
                if modules is not None:
                    for name, param in modules.named_parameters():
                        if "center_loss" in name or "feature_aligner" in name:
                            ...
                            # print(f"[Check!] Detection Head Params are trained")
                            param.requires_grad = (
    ...
    def forward_pts_train(
        ...
        (
            ...
            pts_feats, img_feats, radar_feats, img_metas, is_test=False
        )
        loss_inputs = (
            outputs_classes,
            outputs_coords,
        ...
        losses = self.pts_bbox_head.auxiliary_loss( 
            *loss_inputs,
            gt_bboxes_ignore=gt_bboxes_ignore,
            img_pts_output_dict=img_pts_output_dict,
        )
    ...
    def simple_test_pts(
        ...
        # MK: Time to decouple classification head and localization head
        ...
        (
            ...
            pts_feats, img_feats, radar_feats, img_metas, is_test=True
        )
        ...
        (
            outputs_classes_target,  
            outputs_coords_target,
            aux_outs_loc,
            img_pts_output_dict_loc,
            ncscore_dict_loc,
            p_val_dict_loc,
            center_dict_loc, 
            top3_values_dict_loc,
            top3_indices_dict_loc,
            _, _
        ) = self.pts_bbox_head(
            pts_feats, img_feats, radar_feats, img_metas, is_test=True, is_stage2=True, p_val_dict_next=weight
        )
        
        
        p_val_dict['img5'] = weight["alpha"]
        p_val_dict['pts5'] = weight["beta"]
        
        outs = (outputs_classes_target, outputs_coords_target) 

        bbox_list = self.pts_bbox_head.get_bboxes(*outs, img_metas, rescale=rescale, ncscore_dict=ncscore_dict, p_val_dict=p_val_dict, center_dict=center_dict, top3_values_dict=top3_values_dict, top3_indices_dict=top3_indices_dict)
        bbox_results = [
            bbox3d2result(bboxes, scores, labels)
            for bboxes, scores, labels, topk_ncscore_dict, topk_p_val_dict, topk_center_dict, topk_top3_values_dict, topk_top3_indices_dict in bbox_list
            
            
        ]
        
        return bbox_results, bbox_list[0][3], bbox_list[0][4], bbox_list[0][5], bbox_list[0][6], bbox_list[0][7]
    ...
    def simple_test(self, img_metas, points=None, img=None, radar=None, rescale=False, gt_bboxes_3d=None, gt_labels_3d=None):
        ...
        bbox_pts, topk_ncscore_dict, topk_p_val_dict, topk_center_dict, topk_top3_values_dict, topk_top3_indices_dict = self.simple_test_pts(
